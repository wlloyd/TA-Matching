...
class MatchingGraph:
    ...
    def __init__(self, match_weights: np.ndarray, student_weights, course_info,
        ...
        self.flow = min_cost_flow.SimpleMinCostFlow()

        # Each student cannot fill >1 course slot
        for i, w in enumerate(student_weights):
            self.flow.add_arc_with_capacity_and_unit_cost(
        ...
        for _, row in fixed_matches.iterrows():
            ...
            if si == -1:
                ...
                assign_cost = -int(student_weights.iloc[si] * 10 ** DIGITS)
                self.flow.add_arc_with_capacity_and_unit_cost(
                    si, self.num_students + ci, 1, match_cost + assign_cost)
                self.flow.set_node_supply(si, 1)
        ...
        self.flow.set_node_supply(sink, -int(min(self.num_students, self.slots)))
    ...
    def get_matching(self, fixed_matches: pd.DataFrame, weights: np.ndarray) -> \
        ...
        for arc in range(self.flow.num_arcs()):
    ...
    def graph_weight(self):
        return -self.flow.optimal_cost() / (10 ** DIGITS)
        ...
        for arc in range(self.flow.num_arcs()):
            print(
                "Start: {}, End: {}, Capacity: {}, Cost: {}, flow:{}".format(
                    ...
                    self.flow.capacity(arc), self.flow.unit_cost(arc),
                    self.flow.flow(arc)))
